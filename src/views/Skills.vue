...
export default {
  data() {
    return {
      items: [
        {name: "Python",     imgSrc: require("../assets/Python_light.png"),  time: "2.5年", description: "初めて触った言語で歴は長いです。ハッカソンではFlask, FastAPIをバックエンドでよく使ってます。学生実験ではフレームワークなしでWebサイトを作りました。。。簡単なプログラミング課題はPythonでやることが多いです。最近は機械学習も学んでます。"},
        {name: "Ruby",       imgSrc: require("../assets/Ruby_light.png"),    time: "2年", description: "サークルのアプリ開発でRailsに出会いそれから始めました。インターン先ではDDDに則って開発しています。一番使える言語・フレームワークだと思います。WebAPIをサクッと作成できるくらいにはなりました。"},
        {name: "Vue.js",     imgSrc: require("../assets/Vue_light.png"),     time: "2年", description: "Railsと同じタイミングでフロントエンドとして使い始めました。このポートフォリオもVueで作りました。デザインが苦手なのでVuetify君に頑張ってもらっています。"},
        ...
        {name: "RDB",        imgSrc: require("../assets/rdb_light.png"),     time: "", description: "DBを自分で設計できるくらいには使っています。数万レコードのデータを持つアプリを作成した時には、パフォーマンスを上げるためにN+1問題を解決したり、インデックスを貼ったりしました。"},
        {name: "Docker",     imgSrc: require("../assets/docker_light.png"),  time: "", description: "基本的に開発環境はDockerをつかっています。個人開発でDockerFileからイメージを作ったり本番環境に使ったりしています。"},
      ],
    }
  }
}
